...
class BookingCalendarBlock(models.Model):
    date = models.DateField(unique=True)
    reason = models.CharField(max_length=200, blank=True)
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name='booking_calendar_blocks')
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"Blocked booking date - {self.date}"
