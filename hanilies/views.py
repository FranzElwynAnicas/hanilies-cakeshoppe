...
from .ai_assistant import HANILIES_AI_GREETING, get_hanilies_ai_reply
from .forms import (
    CakeBookingDateForm,
...
from .models import HomeHeroImage, HomeStripImage, UserProfile, Notification, AboutPageImage, BookingCalendarBlock, Cake, CakeOrder, CakeCustomization, Package, PackageOrder, PackageThumbnail, Payment, RefundRequest, ActivityLog, Testimonial, ContactInquiry
...
STOCK_DEDUCTING_ORDER_STATUSES = {
    'confirmed',
    'preparing',
    'ready_for_pickup',
    'out_for_delivery',
...
def hanilies_ai_chat(request):
    ...
    if not message:
        return JsonResponse({
            'reply': HANILIES_AI_GREETING,
            'used_external_ai': False,
            'suggestions': {'cakes': [], 'packages': [], 'booking': None},
        })
    if len(message) > 800:
        return JsonResponse({'error': 'Please keep your message under 800 characters.'}, status=400)

    return JsonResponse(get_hanilies_ai_reply(message))


def hanilies_ai_page(request):
    return render(request, 'hanilies/hanilies_ai.html', {
        'ai_greeting': HANILIES_AI_GREETING,
    })
...
PAYMENT_PROOF_MIN_WIDTH = 300
PAYMENT_PROOF_MIN_HEIGHT = 300
...
PAYMENT_PROOF_ALLOWED_FILE_ERROR = 'Only JPG, JPEG, PNG, and WebP files are allowed.'
DESIGN_REFERENCE_ALLOWED_FILE_ERROR = 'Only JPG, JPEG, PNG, and WebP files are allowed for the design reference.'
...
ADMIN_MENU_ITEMS = [
    ...
    {'name': 'Booking Calendar', 'url': 'admin_booking_calendar', 'icon': 'calendar-days',
        'roles': STAFF_ROLE_VALUES, 'section': 'Orders'},
...
def _validate_checkout_payment_submission(reference_number, proof_image, expected_amount, submitted_amount=None, exclude_payment_id=None):
    ...
    file_extension = os.path.splitext(
        str(getattr(proof_image, 'name', '') or '')
    )[1].lower()
    if file_extension not in PAYMENT_PROOF_ALLOWED_EXTENSIONS:
        return None, PAYMENT_PROOF_ALLOWED_FILE_ERROR
    ...
    if image_format not in PAYMENT_PROOF_ALLOWED_FORMATS:
        return None, PAYMENT_PROOF_ALLOWED_FILE_ERROR
    if image_width < PAYMENT_PROOF_MIN_WIDTH or image_height < PAYMENT_PROOF_MIN_HEIGHT:
        return None, 'The uploaded payment proof image is too small. Please upload a clear receipt image at least 300 x 300 pixels.'
...
def _validate_optional_design_reference_upload(uploaded_image):
    ...
    if content_type and content_type not in PAYMENT_PROOF_ALLOWED_CONTENT_TYPES and content_type not in PAYMENT_PROOF_GENERIC_CONTENT_TYPES:
        return DESIGN_REFERENCE_ALLOWED_FILE_ERROR
...
def _sync_order_confirmation_from_payment(payment, actor=None):
    ...
    if order.order_status in STOCK_DEDUCTING_ORDER_STATUSES:
        _commit_order_stock(order, actor)
...
def _apply_payment_review_action(request_user, payment, action):
    ...
    if action == 'approve':
        if payment.payment_purpose in ['deposit', 'full']:
            order, _order_type = _get_payment_order_details(payment)
            if order is not None and not getattr(order, 'stock_deducted', False):
...
def _apply_order_status_change(request_user, order, order_type, new_status):
    ...
    try:
        if (
            new_status in STOCK_DEDUCTING_ORDER_STATUSES
            and previous_status not in STOCK_DEDUCTING_ORDER_STATUSES
        ):
            _commit_order_stock(order, request_user)
        elif new_status not in STOCK_DEDUCTING_ORDER_STATUSES and getattr(order, 'stock_deducted', False):
...
def _validate_order_stock_availability(order):
    ...
    return (
        f'Cannot confirm order #{order.id} because {product_name} '
...
def _commit_order_stock(order, actor=None):
    ...
    if actor is not None:
        _log_staff_activity(
            ...
            f"Deducted {stock_context['quantity']} stock from {stock_context['product_label']} \"{product.name}\" ({product.product_code or 'Not assigned'}) after confirming {stock_context['order_label']} #{order.id}.",
...
def _restore_order_stock(order, actor=None):
    ...
    if actor is not None:
        _log_staff_activity(
            ...
            f"Restored {stock_context['quantity']} stock to {stock_context['product_label']} \"{product.name}\" ({product.product_code or 'Not assigned'}) after moving {stock_context['order_label']} #{order.id} back to an unconfirmed or cancelled status.",
            stock_context['product_type'],
            product.id,
        )
    return True
...
CUSTOMER_ARCHIVABLE_ORDER_STATUSES = {'completed', 'cancelled', 'refunded'}


def _order_has_processed_refund(order):
    try:
        return order.refund_request.status == 'processed'
    except RefundRequest.DoesNotExist:
        return False


def _can_customer_archive_order(order):
    return (
        order.order_status in CUSTOMER_ARCHIVABLE_ORDER_STATUSES
        or _order_has_processed_refund(order)
    )


def _profile_orders_url():
    return f"{reverse('profile')}?{urlencode({'section': 'orders'})}"
...
def cakes(request):
    ...
    context = {
        ...
        'booking_availability_url': reverse('booking_date_availability'),
    }
...
def booking_date_availability(request):
    date_value = (request.GET.get('date') or '').strip()
    order_type = (request.GET.get('order_type') or '').strip().lower()
    booking_window = build_package_booking_window(
    ) if order_type == 'package' else build_cake_booking_window()
    ...
    if selected_date < booking_window['earliest_date']:
        return JsonResponse({
            'available': False,
            'message': 'This date is too soon for preparation. Please choose a date at least 3 days from today.',
            'min': booking_window['min'],
            'max': booking_window['max'],
        })
    if selected_date > booking_window['latest_date']:
        return JsonResponse({
            'available': False,
            'message': 'Bookings are only accepted up to 30 days in advance.',
            'min': booking_window['min'],
            'max': booking_window['max'],
        })

    availability = get_booking_date_availability(selected_date, order_type)
    return JsonResponse({
        **availability,
        'date': selected_date.isoformat(),
        'min': booking_window['min'],
        'max': booking_window['max'],
        'login_required_to_reserve': not request.user.is_authenticated,
    })
...
def profile(request):
    ...
    visible_cake_orders = cake_orders.filter(customer_archived_at__isnull=True)
    visible_package_orders = package_orders.filter(
        customer_archived_at__isnull=True)
    archived_cake_orders = cake_orders.filter(
        customer_archived_at__isnull=False)
    archived_package_orders = package_orders.filter(
        customer_archived_at__isnull=False)
    total_spent = (
    ...
    def build_order_item(order_type, order):
        if order_type == 'cake':
        ...
        return {
            ...
            'can_customer_archive': _can_customer_archive_order(order),
        }

    recent_orders = sorted(
        [
            build_order_item('cake', order)
            for order in visible_cake_orders[:3]
        ] + [
            build_order_item('package', order)
            for order in visible_package_orders[:3]
        ],
        key=lambda item: item['order'].created_at,
        reverse=True,
    )[:5]
    archived_orders = sorted(
        [
            build_order_item('cake', order)
            for order in archived_cake_orders[:5]
        ] + [
            build_order_item('package', order)
            for order in archived_package_orders[:5]
        ],
        key=lambda item: item['order'].customer_archived_at or item['order'].created_at,
        reverse=True,
    )[:10]
...
@login_required
@require_POST
def customer_order_archive(request, order_type, order_id):
    order = get_object_or_404(
        _get_customer_order_queryset(request.user, order_type),
        id=order_id,
    )

    if not _can_customer_archive_order(order):
        messages.error(
            request,
            'Only completed, cancelled, or refunded orders can be archived.',
        )
        return redirect(_profile_orders_url())

    if order.customer_archived_at is None:
        order.customer_archived_at = timezone.now()
        order.save(update_fields=['customer_archived_at'])
        messages.success(request, 'Order archived from your Recent Orders.')
    else:
        messages.info(request, 'This order is already archived.')
...
@login_required
@require_POST
def customer_order_unarchive(request, order_type, order_id):
    order = get_object_or_404(
        _get_customer_order_queryset(request.user, order_type),
        id=order_id,
    )

    if order.customer_archived_at is not None:
        order.customer_archived_at = None
        order.save(update_fields=['customer_archived_at'])
        messages.success(request, 'Order restored to your Recent Orders.')
    else:
        messages.info(request, 'This order is already in your Recent Orders.')
...
def _build_booking_calendar_day_rows(month_start):
    days_in_month = calendar.monthrange(month_start.year, month_start.month)[1]
    active_statuses = ['pending', 'payment_retry', 'confirmed',
    ...
    for day_number in range(1, days_in_month + 1):
        ...
        day_rows.append({
            'date': current_date,
            'availability': get_booking_date_availability(current_date, 'all'),
            'block': BookingCalendarBlock.objects.filter(date=current_date).first(),
            'cake_orders': CakeOrder.objects.select_related('user', 'cake').filter(
                delivery_date__date=current_date,
                order_status__in=active_statuses,
                is_archived=False,
            ).order_by('delivery_date', 'id'),
            'package_orders': PackageOrder.objects.select_related('user', 'package').filter(
...
def admin_booking_calendar(request):
    ...
    try:
        month_start = datetime.strptime(
            selected_month, '%Y-%m').date().replace(day=1)
    except (TypeError, ValueError):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect(f"{reverse('admin_booking_calendar')}?month={month_start.strftime('%Y-%m')}")

        if action == 'block':
            block, created = BookingCalendarBlock.objects.update_or_create(
                date=block_date,
                defaults={
                    'reason': reason or 'Unavailable for shop scheduling.',
                    'created_by': request.user,
                },
            )
            _log_staff_activity(
                request.user,
                'booking_date_blocked' if created else 'booking_date_block_updated',
                f'Blocked booking date {block_date.isoformat()}.',
                target_type='booking_calendar_block',
                target_id=block.id,
            )
            messages.success(request, 'Booking date block was saved.')
        elif action == 'unblock':
            deleted_count, _ = BookingCalendarBlock.objects.filter(
                date=block_date).delete()
            if deleted_count:
                _log_staff_activity(
                    request.user,
                    'booking_date_unblocked',
                    f'Reopened booking date {block_date.isoformat()}.',
                    target_type='booking_calendar_block',
                )
                messages.success(request, 'Booking date was reopened.')
            else:
                messages.info(request, 'That date was not blocked.')
        else:
            messages.error(
                request, 'Please choose a valid booking calendar action.')

        return redirect(f"{reverse('admin_booking_calendar')}?month={block_date.strftime('%Y-%m')}")
    ...
    return render(request, 'admin/booking_calendar.html', {
        ...
        'day_rows': _build_booking_calendar_day_rows(month_start),
        'daily_booking_limit': ORDER_DAILY_BOOKING_LIMIT,
        'cake_daily_order_limit': CAKE_DAILY_ORDER_LIMIT,
        'package_daily_event_limit': PACKAGE_DAILY_EVENT_LIMIT,
        'upcoming_cake_orders': CakeOrder.objects.select_related('user', 'cake').filter(
            delivery_date__date__gte=today,
            order_status__in=active_statuses,
            is_archived=False,
        ).order_by('delivery_date', 'id')[:10],
        'upcoming_package_orders': PackageOrder.objects.select_related('user', 'package').filter(
            event_date__gte=today,
            order_status__in=active_statuses,
            is_archived=False,
        ).order_by('event_date', 'event_time', 'id')[:10],
    })
...
def admin_dashboard(request):
    ...
    quick_actions.append({
        ...
        'copy': 'Check date capacity, pending bookings, and blocked days.',
        'icon': 'calendar-days',
        'url': reverse('admin_booking_calendar'),
        'priority': False,
    })
    if can_view_cakes:
        quick_actions.append({
...
def admin_payment_verify(request, payment_id):
    ...
    if request.method == 'POST':
        ...
        try:
            _apply_payment_review_action(
                request.user, payment, normalized_action)
